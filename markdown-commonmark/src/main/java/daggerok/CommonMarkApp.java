...
class CommonMarkConfig {
  ...
  @Bean
  MarkdownRenderer markdownRenderer(MarkdownToHtml markdownToHtml) {
    return path -> Try.of(() -> ResourceUtils.getFile("classpath:/content/" + path))
                      .map(File::toPath)
                      .mapTry(p -> Files.lines(p, StandardCharsets.UTF_8))
                      .map(stringStream -> stringStream.collect(Collectors.joining()))
                      .map(markdownToHtml)
                      .getOrElseGet(err -> String.format("<html><body><h1>Rendering error</h1><p>%s</p></body></html>",
                                                         err.getLocalizedMessage()));
  }*/
}
...
public class CommonMarkApp {
  ...
  public static void main(String[] args) {
    SpringApplication.run(CommonMarkApp.class, args);
  }
}
